...
import { ProfileAvatar, ProductImage, type LayoutProps, formatPrice } from "./shared";
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-3 px-4 py-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">@{profileData.slug || "username"} · 추천 상품 {links.length}개</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 px-2 pb-4">
        {links.map((link, i) => {
          const tall = i % 3 === 0;
          return (
            <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className={cn("group relative overflow-hidden rounded-lg", !link.enabled && "opacity-40")}>
              <div className={cn("relative flex items-center justify-center overflow-hidden", tall ? "h-48" : "h-32")}>
                <ProductImage src={link.imageUrl} className="size-full text-[8px]" />
              </div>
