      ...
      <div className="mt-4 flex flex-col gap-2 px-4 pb-6">
        <HighlightWrapper section="links">
          <div className="flex flex-col gap-2">
            {links.map((link) => (
              <a key={link.id} href={link.url} onClick={(e) => handleLinkClick(e, link)} target="_blank" rel="noopener noreferrer" className={cn("group flex items-center gap-3 border border-border p-2.5 transition-colors hover:border-primary/30", getLinkRoundClass(profileData.linkRound), !link.enabled && "opacity-40")} style={getLinkBorderStyle(profileData)}>
                ...
                <div className="min-w-0 flex-1">
                  <p className={cn("truncate text-sm font-semibold", !link.title && "opacity-40")}>{link.title || "상품명을 입력하세요"}</p>
                  {link.price != null ? <PriceTag link={link} /> : <p className="text-xs opacity-30">가격 미입력</p>}
